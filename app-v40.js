...
function renderYearModules(){
  const container=$("yearModules");
  const select=$("yearFilter");
  if(!container||!select)return;

  let filter=String(select.value||"1");
  if(!["all","1","2","3","4","5"].includes(filter)){
    filter="1";
    select.value="1";
  }

  const currentSubjects=Array.isArray(state.subjects)?state.subjects:[];
  const years=pharmacyYears.filter(function(y){
    return filter==="all"||String(y.year)===filter;
  });

  let html="";
  years.forEach(function(y){
    html+='<article class="curriculum-year-card">';
    html+='<div class="curriculum-year-header">';
    html+='<h2>'+esc(y.title)+'</h2>';
    html+='<span class="year-badge">Year '+esc(y.year)+'</span>';
    html+='</div>';

    y.semesters.forEach(function(sem,index){
      const open=index===0;
      html+='<section class="semester-accordion">';
      html+='<button class="semester-toggle '+(open?'open':'')+'" type="button" onclick="toggleSemester(this)">';
      html+='<span>'+esc(sem.name)+'</span>';
      html+='<span><small>'+esc(sem.credits)+' ساعة</small> <b class="semester-arrow">⌄</b></span>';
      html+='</button>';
      html+='<div class="semester-courses '+(open?'open':'')+'">';

      sem.courses.forEach(function(c){
        const added=currentSubjects.some(function(s){return s&&s.name===c.name});
        const safeName=esc(c.name).replace(/'/g,"&#39;");
        const safeCode=esc(c.code||"").replace(/'/g,"&#39;");
        html+='<div class="curriculum-course-row">';
        html+='<button class="course-add-btn '+(added?'added':'')+'" type="button" onclick="addCurriculumSubject(\''+safeName+'\',\''+safeCode+'\','+Number(c.credits||0)+')" title="إضافة المادة">'+(added?'✓':'+')+'</button>';
        html+='<button class="course-name-button" type="button" onclick="previewCurriculumCourse(\''+safeName+'\',\''+esc(y.year)+'\')">'+esc(c.name)+'</button>';
        html+='<div class="course-credit">'+esc(c.credits)+'<small>ساعة</small></div>';
        html+='</div>';
      });
    ...
    html+='<div class="curriculum-note">';
    (y.focus||[]).forEach(function(item){html+='• '+esc(item)+'<br>'});
    html+='</div></article>';
  });

  container.innerHTML=html||'<div class="medical-disclaimer">لم يتم العثور على مواد لهذه السنة. أعيدي تحميل الصفحة.</div>';
}
...
async function renderSubjectFiles(){
  ...
  const cloudHtml=await renderCloudSubjectFiles();
  ...
  </div>`).join(""):'<div class="muted">لم تتم إضافة ملفات محلية لهذه المادة بعد.</div>';
  $("subjectFilesList").innerHTML+=`<h4 style="margin-top:16px">الملفات السحابية</h4>${cloudHtml||'<div class="muted">سجّلي الدخول لعرض الملفات السحابية.</div>'}`;
}
...
async function uploadWorkspaceFilesToCloud(files){
  if(!activeSubjectId)return;
  if(!window.FocusBloomCloud?.configured()){toast("فعّلي Supabase أولًا");return}
  try{
    for(const file of files)await window.FocusBloomCloud.uploadSubjectFile(activeSubjectId,file);
    toast("تم رفع الملفات إلى السحابة");
    await renderSubjectFiles();
  }catch(e){toast(e.message)}
}
async function renderCloudSubjectFiles(){
  if(!activeSubjectId||!window.FocusBloomCloud?.configured())return "";
  try{
    const user=await window.FocusBloomCloud.getUser();
    if(!user)return "";
    const records=await window.FocusBloomCloud.listSubjectFiles(activeSubjectId);
    if(!records.length)return '<div class="muted">لا توجد ملفات سحابية لهذه المادة.</div>';
    return records.map(r=>`<div class="file-row">
      <div><strong>☁️ ${esc(r.file_name)}</strong><div class="file-meta">${Math.ceil((r.size_bytes||0)/1024)} KB · سحابي</div></div>
      <div class="hero-actions">
        <button class="text-btn" onclick="analyzeCloudFile('${r.id}')">تحليل</button>
        <button class="danger-btn" onclick="removeCloudFile('${r.id}')">حذف</button>
      </div>
    </div>`).join("");
  }catch(e){return `<div class="muted">تعذر تحميل الملفات السحابية: ${esc(e.message)}</div>`}
}
window.analyzeCloudFile=async id=>{
  try{
    const records=await window.FocusBloomCloud.listSubjectFiles(activeSubjectId);
    const record=records.find(x=>x.id===id);if(!record)return;
    const file=await window.FocusBloomCloud.downloadSubjectFile(record);
    nav("course-analyzer");
    const s=state.subjects.find(x=>x.id===activeSubjectId);
    $("courseName").value=s?.name||"";
    const dt=new DataTransfer();dt.items.add(file);$("courseFile").files=dt.files;
    toast("تم نقل الملف السحابي إلى محلل المادة");
  }catch(e){toast(e.message)}
};
window.removeCloudFile=async id=>{
  try{
    const records=await window.FocusBloomCloud.listSubjectFiles(activeSubjectId);
    const record=records.find(x=>x.id===id);if(!record)return;
    await window.FocusBloomCloud.deleteSubjectFile(record);
    toast("تم حذف الملف السحابي");renderSubjectFiles();
  }catch(e){toast(e.message)}
};
...
/* ---------- FocusBloom 4.0 setup center ---------- */
function setupFeatureRows(){
  const cfg=window.FocusBloomCloud?.config?.()||{};
  const connected=window.FocusBloomCloud?.configured?.();
  const rows=[
    ["Project URL",Boolean(cfg.SUPABASE_URL)],
    ["Publishable key",Boolean(cfg.SUPABASE_PUBLISHABLE_KEY)],
    ["Supabase client",Boolean(connected)],
    ["OPENAI_API_KEY","server"],
    ["Database tables","manual"],
    ["Edge Functions","manual"]
  ];
  $("featureChecklist").innerHTML=rows.map(([name,status])=>{
    const icon=status===true?"✅":status==="server"?"🔐":"⬜";
    const text=status===true?"جاهز":status==="server"?"محفوظ في Secrets":"يحتاج تنفيذ";
    return `<div class="setup-row"><span>${icon} ${esc(name)}</span><strong>${text}</strong></div>`;
  }).join("");
  $("setupConnectionBadge").textContent=connected?"مهيّأ":"غير مهيّأ";
}
function restoreRuntimeConfigForm(){
  const cfg=window.FocusBloomCloud?.config?.()||{};
  $("runtimeSupabaseUrl").value=cfg.SUPABASE_URL||"";
  $("runtimeSupabaseKey").value=cfg.SUPABASE_PUBLISHABLE_KEY||"";
  setupFeatureRows();
}
async function saveAndTestRuntimeConfig(e){
  e.preventDefault();
  $("setupTestResult").innerHTML="<p>جارٍ الاختبار…</p>";
  try{
    window.FocusBloomCloud.savePublicConfig(
      $("runtimeSupabaseUrl").value,
      $("runtimeSupabaseKey").value
    );
    const result=await window.FocusBloomCloud.testConnection();
    $("setupTestResult").innerHTML=`<p>✅ تم الاتصال بمشروع Supabase بنجاح.</p><p class="muted">${result.hasSession?"يوجد مستخدم مسجل حاليًا.":"الخطوة التالية: سجلي الدخول من صفحة الحساب والمزامنة."}</p>`;
    setupFeatureRows();updateCloudStatus();
  }catch(e){
    $("setupTestResult").innerHTML=`<p>❌ ${esc(e.message)}</p>`;
    setupFeatureRows();
  }
}
function clearRuntimeConfiguration(){
  if(!confirm("مسح Project URL وPublishable key من هذا الجهاز؟"))return;
  window.FocusBloomCloud.clearPublicConfig();
  restoreRuntimeConfigForm();
  $("setupTestResult").innerHTML="<p>تم مسح الإعداد من هذا الجهاز.</p>";
}

document.querySelectorAll(".nav-link").forEach(n=>n.classList.toggle("active",n.dataset.page===page));const titles={dashboard:"مرحبًا بكِ في FocusBloom Pharmacy 👋",timer:"جلسة تركيز",subjects:"مواد الصيدلة","drug-vault":"Drug Vault",flashcards:"Flashcards",quiz:"Quiz Generator",planner:"Study Planner",trainer:"Pharmacy Trainer",stats:"الإحصائيات",achievements:"الإنجازات",assistant:"مساعد الصيدلة",setup:"الإعداد والتفعيل","subject-workspace":"ملفات المادة","drug-explorer":"Drug Explorer",curriculum:"Pharmacy Years Hub","course-analyzer":"محلل ملفات المواد",cloud:"الحساب والمزامنة",profile:"الملف الشخصي"};$("pageTitle").textContent=titles[page]||"FocusBloom";window.scrollTo({top:0,behavior:"smooth"});if(page==="flashcards")startReview();if(page==="trainer")startTrainer()}
...
$("todayDate").textContent=new Date().toLocaleDateString("ar-JO",{weekday:"long",year:"numeric",month:"long",day:"numeric"})+" · الإصدار 4.0";
...
$("runtimeConfigForm").onsubmit=saveAndTestRuntimeConfig;
$("clearRuntimeConfig").onclick=clearRuntimeConfiguration;
$("backToSubjects").onclick=()=>nav("subjects");
$("subjectFileUpload").onchange=async e=>{
  if(!activeSubjectId)return;
  const selected=[...e.target.files];
  for(const file of selected)await saveSubjectFile(activeSubjectId,file);
  if(window.FocusBloomCloud?.configured()&&await window.FocusBloomCloud.getUser()){
    if(confirm("تم الحفظ على الجهاز. هل تريدين رفع نسخة سحابية أيضًا؟"))await uploadWorkspaceFilesToCloud(selected);
  }
...
render();updateTimer();startReview();startTrainer();updateCloudStatus();renderYearModules();restorePlanSettings();restoreRuntimeConfigForm();
